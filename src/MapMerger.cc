...
// MapMerger::Map* MapMerger::MergeMaps(Map* pMapCurr, Map* pMapMatch, vector<MapMatchHit> vMatchHits)
// {
...
//     //create new map
//     Map* pFusedMap{new Map(pMapMatch,pMapCurr)};//前者应该是此前已有的地图，后者是当前的地图，要把当前地图融合到此前地图中去，成为一个新地图
...
//     int idx = 0;
//     KeyFrame* pKFCur = vMatchHits[idx].mpKFCurr;
//     KeyFrame* pKFMatch = vMatchHits[idx].mpKFMatch;
//     g2o::Sim3 g2oScw = vMatchHits[idx].mg2oScw;
//     std::vector<MapPoint*> vpCurrentMatchedPoints = vMatchHits[idx].mvpCurrentMatchedPoints;
//     std::vector<MapPoint*> vpLoopMapPoints = vMatchHits[idx].mvpLoopMapPoints;

//     vector<KeyFrame*> vpKeyFramesCurr = pMapCurr->GetAllKeyFrames();
...
//     for(vector<KeyFrame*>::iterator vit = vpKeyFramesCurr.begin();vit!=vpKeyFramesCurr.end();++vit)
//     {
//         KeyFrame* pKFi = *vit;
...
//     map<idpair,KeyFrame*>  mpErasedKFs = pMapCurr->GetMmpErasedKeyFrames();
//     map<idpair,MapPoint*>  mpErasedMPs = pMapCurr->GetMmpErasedMapPoints();

//     for(map<idpair,KeyFrame*>::iterator mitEr = mpErasedKFs.begin();mitEr != mpErasedKFs.end();++mitEr)
//     {
//         KeyFrame* pKFi = mitEr->second;
...
//     for(map<idpair,MapPoint*>::iterator mitEr = mpErasedMPs.begin();mitEr != mpErasedMPs.end();++mitEr)
//     {
//         MapPoint* pMPi = mitEr->second;
...
//         {
//             MapPoint* pLoopMP = vpCurrentMatchedPoints[i];
//             MapPoint* pCurMP = pKFCur->GetMapPoint(i);
//             if(pCurMP)
//             {
...
// void MapMerger::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, std::vector<MapPoint*> vpLoopMapPoints)
// {
...
//         vector<MapPoint*> vpReplacePoints(vpLoopMapPoints.size(),nullptr);
...
//             if(pRep)
//             {
//                 // pRep->ReplaceAndLock(vpLoopMapPoints[i]);//todo
//             }
//         }
//     }
// }
...
// void MapMerger::RunGBA(idpair nLoopKf, Map* pFusedMap)
// {
...
//         list<KeyFrame*> lpKFtoCheck(pFusedMap->mvpKeyFrameOrigins.begin(),pFusedMap->mvpKeyFrameOrigins.end());
...
//         // Correct MapPoints
//         const vector<MapPoint*> vpMPs = pFusedMap->GetAllMapPoints();
...
// //                    KeyFrame* pKFp = pRefKF->GetParent();
