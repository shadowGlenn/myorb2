...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        if(mbActivateLocalizationMode)//激活纯定位模式
        {
            mpLocalMapper->RequestStop();
        ...
        if(mbDeactivateLocalizationMode)//关闭纯定位模式,即除了跟踪还要建图，纯定位则是只有跟踪
        {
            //todo：comment by chj
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();
...
void System::SaveMap0(const string &filename)
{
    mpMap0->Save(filename);
}
void System::SaveMap1(const string &filename)
{
    mpMap1->Save(filename);
}
...
void System::LoadMap(const string &filename)
{
    ...
    // ActivateLocalizationMode();
     ...
     mpMap->Load(filename,mySystemSetting,mpKeyFrameDatabase);//注意在这一步要先选择单步调试，viewer才能正常显示
}
void System::LoadMap0(const string &filename)
{
     SystemSetting* mySystemSetting = new SystemSetting(mpVocabulary);
    ...
    // ActivateLocalizationMode();
     ...
     mpMap0->Load(filename,mySystemSetting,mpKeyFrameDatabase);//注意在这一步要先选择单步调试，viewer才能正常显示
}
void System::LoadMap1(const string &filename)
{
     SystemSetting* mySystemSetting = new SystemSetting(mpVocabulary);
    ...
    // ActivateLocalizationMode();
     ...
     mpMap1->Load(filename,mySystemSetting,mpKeyFrameDatabase);//注意在这一步要先选择单步调试，viewer才能正常显示
}

// void System::InitializeMaps()
// {
//     //只有一台client，orb2中是mpMap = new Map();
//     mpMap0.reset(new Map);
//     //有2台client
//         // if(mNumOfClients > 1)     
//     // mpMap1.reset(new Map(1));
//         // else mpMap1=nullptr;
