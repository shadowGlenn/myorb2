...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    // Step 1：找出和当前帧具有公共单词的所有关键帧，不包括与当前帧连接的关键帧
    {
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            ...
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];
            // 然后对这些关键帧展开遍历，也就是说这些关键帧都含有相同的某个word，而这个word的id是vit->first
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                KeyFrame* pKFi=*lit;
                ...
                if(pKFi->mnLoopQuery!=pKF->mnId)    
                {
                    ...
                    if(!spConnectedKeyFrames.count(pKFi))
                    {
                        ...
                        pKFi->mnLoopQuery=pKF->mnId;//表示当前关键帧pKFi是id为mnId的pKF的闭环候选关键帧，就这样逐个遍历pKFi，最后就能得到pKF的闭环候选关键帧列表
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    // vector<KeyFrameDatabase::KeyFrame*> KeyFrameDatabase::DetectMapMatchCandidates(KeyFrame* pKF, float minScore, Map* pMap)
    vector<KeyFrame*> KeyFrameDatabase::DetectMapMatchCandidates(KeyFrame* pKF, float minScore, Map* pMap)//用上面ccm的名字会报错KeyFrame’ is not a member of ‘ORB_SLAM2::KeyFrameDatabase’，因此参照原版orb的DetectLoopCandidates来写。todo：前者返回的是 KeyFrameDatabase 类中的 KeyFrame 类指针，后者返回的是外部声明的 KeyFrame 类指针。不知道有没有影响。
    {
        list<KeyFrame*> lKFsSharingWords;

        // Search all keyframes that share a word with current keyframes
        // Discard keyframes that belong to KF's map：因为这里是地图匹配，所以那些属于同一个map的kf要丢弃，应该在闭环检测中处理。
        {
            unique_lock<mutex> lock(mMutex);
            ...
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++)
            {
                ...
                for (list<KeyFrame*>::iterator lit = lKFs.begin(), lend = lKFs.end(); lit != lend; lit++)
                {
                    ...
                    if (!(pKFi->mMatchQuery == pKF->mId))
                    {
                        ...
                        if (!pMap->msuAssClients.count(pKFi->mId.second))//如果当前帧所属地图的关联客户端X（X是pKFi->mId.second，即
                        {
