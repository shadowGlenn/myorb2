...
bool LoopClosing::DetectLoop()
{
    ...
    // 后者的体现是当mLastLoopKFid为0的时候，chj：当mLastLoopKFid为0时，如果mpCurrentKF小于10,说明当前地图中的关键帧小于10；那如果mLastLoopKFid不为0呢？后续    mLastLoopKFid 会被 mpCurrentKF->mnId更新，并且mpCurrentKF不会小于10,否则mLastLoopKFid就不会被更新。所以地图中小于10帧的应该不会出现，只在系统刚开始时有可能。
...
void LoopClosing::CorrectLoop()
{
    ...
    // 对地图点操作
    {
        ...
        // Step 2.1：通过mg2oScw（认为是准的）来进行位姿传播，得到当前关键帧的共视关键帧（当前关键帧组）的世界坐标系下Sim3 位姿（即矫正后的位姿）
        // 遍历"当前关键帧组""
    ...
    // Fuse duplications.
    // Step 4：将闭环相连关键帧组 mvpLoopMapPoints 投影到当前关键帧组中，进行匹配，融合，新增或替换当前关键帧组中KF的地图点,mvpLoopMapPoints:vpLoopConnectedKFs的地图点
...
/**
 * @brief 将闭环相连关键帧组（the neighborhood of the loop keyframe）的mvpLoopMapPoints 投影到当前关键帧组中，进行匹配，新增或替换当前关键帧组中KF的地图点
 * 因为 闭环相连关键帧组mvpLoopMapPoints 在地图中时间比较久经历了多次优化，认为是准确的
 * 而当前关键帧组中的关键帧的地图点是最近新计算的，可能有累积误差
 * 
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        // mvpLoopMapPoints：与当前关键帧闭环匹配上的关键帧及其共视关键帧（the neighborhood of the loop keyframe）中的地图点
