...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    // 如果已经添加过观测，返回
    if(mObservations.count(pKF)) //当前地图点的观测列表中，
        return;
...
void MapPoint::SetBadFlag()
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        pKF->EraseMapPointMatch(mit->second);//mit->second:地图点在该关键帧中的id
    }
    // 擦除该MapPoint申请的内存
...
void MapPoint::Replace(MapPoint* pMP)
{
    ...
    //- 将观测到当前地图点的关键帧的信息进行更新
    //obs应该是指观测到当前地图点的所有关键帧，obs为map类型，first表示关键帧，second表示当前地图点在该关键帧中的索引
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        // Replace measurement in keyframe
        KeyFrame* pKF = mit->first;

        if(!pMP->IsInKeyFrame(pKF))//要用来替换的地图点不在当前操作的这个关键帧pKF中；如果在的话，也就是说其信息已经有了，就不用进行操作了（但是对于被替换的地图点，要删除其在关键帧中的观测）
        {   
        ...
        else
        {
            ...
            //说白了,既然是让对方的那个地图点来代替当前的地图点,就是说明对方更好,所以删除这个关键帧对当前点的观测
