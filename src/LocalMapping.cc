 ...
 * 冗余关键帧的判定：该关键帧中90%以上的地图点能被其他关键帧（至少3个）观测到
 */
void LocalMapping::KeyFrameCulling()
{
    ...
    // 该函数里变量层层深入，这里列一下：
    // mpCurrentKeyFrame：当前关键帧，本程序就是判断它是否需要删除，应该是删除其共视关键帧pKF吧？
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>thObs)//当前地图点被超过3个的关键帧观测到了，但是此处的Observations应该是包含了当前关键帧本身的，但我们要判断的是其他关键帧的数量是否超过3个
                    {
                        ...
                        int nObs=0;
                        // 遍历观测到该地图点的关键帧，含当前关键帧本身
                        for(map<KeyFrame*, size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            KeyFrame* pKFi = mit->first;
                            if(pKFi==pKF)//排除当前关键帧本身
                                continue;
                            const int &scaleLeveli = pKFi->mvKeysUn[mit->second].octave;

                            // 尺度约束：为什么pKF 尺度+1 要大于等于 pKFi 尺度？
                            // 回答：因为同样或更低金字塔层级的地图点更准确，不满足这个条件的就不会被计入
                            if(scaleLeveli<=scaleLevel+1)//应该就是说，金字塔层数相差超过2级的，即使被观测到了，也不能算数，因为不准确
                            {
                                nObs++;
